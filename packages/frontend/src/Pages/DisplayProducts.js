...
export default function DisplayProducts() {
  ...
  return (
    <div className=" w-full flex justify-between  flex-col items-center gap-11 min-h-screen   bg-yellow-200">
      <div className="w-10/12 h-screen min-h-screen justify-center  grid grid-cols-5 grid-rows-2 items-center">
        {currentPosts.map((item) => {
          return (
            <div
              key={item.id}
              className="w-44 h-44 mb-4 bg-blue-500 bottom-2 border-white"
            >
              <h2>{item.name}</h2>
            </div>
          );
          ...
          return (
            <li
              ...
              className={`w-8 h-8 rounded-full text-center ${
                currentPage === item ? "bg-black text-white" : "bg-white"
              }`}
            >
              <a
                href="!#"
